...
<body>
    ...
    <script>
        ...
        const csvContent = "data:text/csv;charset=utf-8," + 
                "Producto,Categoria,Estado,Cantidad\n" +
                products.map(p => 
                    `"${p.name}","${p.category}","${p.state}",${p.quantity}`
                ).join("\n");
                    ...
                    try {
                        if (file.name.endsWith('.json')) {
                            ...
                            if (data.products && Array.isArray(data.products)) {
                                // Extract only required fields from each product
                                ...
                                if (confirm(`¿Deseas importar ${simplifiedProducts.length} productos? Esto reemplazará los datos actuales.`)) {
