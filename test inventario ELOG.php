...
<head>
    ...
    <style>
        * {
        ...
        .header-content {
            margin-left: 30px;
            order: 2; /* This will push the content to the right */
        }
        ...
        .Logo {
            order: 1; /* This will keep the logo on the left */
        }
...
<body>
    <div class="container">
        ...
        <div class="stats">
            ...
            <div class="stat-card">
                <div class="stat-number" id="Transit">0</div>
        ...
        <div class="controls">
            <div class="controls-row">
                ...
                <button class="btn btn-secondary" onclick="clearAllData()" style="background: linear-gradient(135deg, #ff6b6b, #ee5a52); color: white;">
                    🗑️ Limpiar Todo
                </button>
                 -->
            </div>
    ...
    <!-- ADDING PRODUCTS -->
                    ...
                    <div class="form-group">
                        <label for="productQuantity">Disponible *</label>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="transit" min="0" required>
    ...
    <script>
        ...
        const stockThresholds = {
            ...
            // Get thresholds for a specific product
            getThresholds: function(product) {
                // Check if there are specific thresholds for this category
                if (this.categories[product.category]) {
                    return this.categories[product.category];
                }
                // Return default thresholds if no specific ones found
                return this.default;
            }
        };

        // Server-backed storage: use Database/products_api.php
        let editingId = null; // store product id while editing

        async function loadFromServer() {
            try {
                const res = await fetch('Database/products_api.php?action=list');
                ...
                if (data.success) {
                    products = data.products.map(p => ({
                        ...
                        transit: parseInt(p.transit) || 0,
                        price: parseFloat(p.price) || 0,
                        description: p.description || ''
                    }));
                    updateLastSavedIndicator(new Date().toISOString());
                    showLoadNotification(products.length);
                    updateDisplay();
                    return true;
                } else {
                    console.error('Failed to load from server', data);
                }
            } catch (e) {
        ...
        async function apiAddProduct(product) {
            const res = await fetch('Database/products_api.php?action=add', {
                method: 'POST',
        ...
        async function apiUpdateProduct(product) {
            const res = await fetch('Database/products_api.php?action=update', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify(product)
            });
        ...
        async function apiDeleteProduct(id) {
            const res = await fetch('Database/products_api.php?action=delete', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
        ...
        function initializeSampleData() {
            products = [];
            updateDisplay();
            // No local storage by default; you can optionally add server-side sample items here
            // Example:
            // apiAddProduct({name: 'Ejemplo', category: 'ELOG', state: 'Nuevo', quantity: 10, transit: 0, price: 5.00, description: ''});
        }

        async function initializeApp() {
            // Load products from server; fallback to sample data if offline
        ...
        function updateStats() {
            ...
            // Count products with low stock using different rules for transit/no-transit
            const lowStockCount = products.filter(p => {
                if (p.transit) {
                    const maxQuantity = p.quantity + p.transit;
                    return p.quantity <= (maxQuantity * 0.5);
                } else {
                    const thresholds = stockThresholds.getThresholds(p);
                    return p.quantity <= thresholds.low; // Use configured threshold
                }
            }).length;
            const totalValue = products.reduce((sum, p) => sum + (p.quantity * p.price), 0);
            const totalTransit = products.reduce((sum, p) => sum + (parseInt(p.transit) || 0), 0);
            ...
            document.getElementById('Transit').textContent = totalTransit;
        }
        ...
        function updateTable() {
            ...
            let filteredProducts = products.filter(product => {
                ...
                // Different thresholds based on whether there's transit or not
                if (product.transit) {
                    const maxQuantity = product.quantity + product.transit;
                    if (stockFilter === 'low') matchesStock = product.quantity <= (maxQuantity * 0.5);
                    else if (stockFilter === 'medium') matchesStock = product.quantity > (maxQuantity * 0.5) && product.quantity <= (maxQuantity * 0.75);
                    else if (stockFilter === 'high') matchesStock = product.quantity > (maxQuantity * 0.70);
                } else {
                    // When no transit, use configured thresholds
                    const thresholds = stockThresholds.getThresholds(product);
                    if (stockFilter === 'low') matchesStock = product.quantity <= thresholds.low;
                    else if (stockFilter === 'medium') matchesStock = product.quantity > thresholds.low && product.quantity <= thresholds.medium;
                    else if (stockFilter === 'high') matchesStock = product.quantity > thresholds.medium;
                }
            ...
            tbody.innerHTML = filteredProducts.map((product, index) => {
                ...
                if (product.transit) {
                    const maxQuantity = product.quantity + product.transit;
                    stockClass = product.quantity <= (maxQuantity * 0.5) ? 'stock-low' : 
                               product.quantity <= (maxQuantity * 0.70) ? 'stock-medium' : 'stock-good';
                } else {
                    const thresholds = stockThresholds.getThresholds(product);
                    stockClass = product.quantity <= thresholds.low ? 'stock-low' : 
                               product.quantity <= thresholds.medium ? 'stock-medium' : 'stock-good';
                               product.quantity <= 50 ? 'stock-medium' : 'stock-good';
                }
                const total = product.quantity * product.price;
                const transit = product.transit
                ...
                return `
                    <tr>
                        ...
                        <td>
                            ...
                            <button class="btn btn-danger" onclick="deleteProduct(${originalIndex})" 
        ...
        function openModal(index = -1) {
            ...
            if (index >= 0) {
                ...
                document.getElementById('transit').value = product.transit;
        ...
        async function deleteProduct(index) {
            ...
            if (confirm('¿Estás seguro de que deseas eliminar este producto?')) {
                const resp = await apiDeleteProduct(product.id);
                if (resp.success) {
                    // remove locally and refresh
                    products.splice(index, 1);
                    updateDisplay();
                } else {
                    alert('Error al eliminar: ' + (resp.error || resp.message || 'Unknown'));
                }
            }
        }
        ...
        const csvContent = "data:text/csv;charset=utf-8," + 
                "Nombre,Categoría,Cantidad,Transit,Precio,Total,Descripción\n" +
                products.map(p => 
                    `"${p.name}","${p.category}",${p.quantity},${p.transit},${p.price},${p.quantity * p.price},"${p.description || ''}"`
                ).join("\n");
                    ...
                    try {
                        if (file.name.endsWith('.json')) {
                            ...
                            if (data.products && Array.isArray(data.products)) {
                                if (confirm(`¿Deseas importar ${data.products.length} productos? Esto agregará los productos al inventario.`)) {
                                    (async () => {
                                        for (const p of data.products) {
                                            ...
                                            const payload = {
                                                ...
                                                quantity: parseInt(p.quantity || p.cantidad || 0) || 0,
                                                transit: parseInt(p.transit || 0) || 0,
                                                price: parseFloat(p.price || p.precio || 0) || 0,
        ...
        function clearAllData() {
            if (confirm('⚠️ ¿Estás seguro de que deseas eliminar TODOS los productos?\n\nEsta acción no se puede deshacer.')) {
                (async () => {
                    try {
                        // fetch latest products and delete each
                        await loadFromServer();
                        ...
                        setTimeout(() => alert('✅ Todos los datos han sido eliminados.'), 100);
                    } catch (err) {
        ...
        document.getElementById('productForm').addEventListener('submit', function(e) {
            ...
            const payload = {
                ...
                transit: parseInt(document.getElementById('transit').value) || 0,
            ...
            (async () => {
                try {
                    if (editingId) {
                        ...
                        const resp = await apiUpdateProduct(payload);
                        if (resp.success) {
                            // update local copy
                            const idx = products.findIndex(p => p.id == resp.product.id);
                            if (idx >= 0) products[idx] = {
                                id: resp.product.id,
                                name: resp.product.name,
                                category: resp.product.category,
                                state: resp.product.state,
                                quantity: parseInt(resp.product.quantity) || 0,
                                transit: parseInt(resp.product.transit) || 0,
                                price: parseFloat(resp.product.price) || 0,
                                description: resp.product.description || ''
                            };
                        } else {
                            alert('Error updating: ' + (resp.error || resp.message || 'Unknown'));
                        }
                    } else {
                        const resp = await apiAddProduct(payload);
                        if (resp.success && resp.product) {
                            ...
                            alert('Error adding product: ' + (resp.error || resp.message || 'Unknown'));
                        }
                    }
                    updateDisplay();
                    document.getElementById('productModal').style.display = 'none';
                    ...
                    alert('Error saving product. Revisa la consola.');
                }
            })();
        });
